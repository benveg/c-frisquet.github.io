...
var resizePizzas = function(size) {
  ...
  //But now without a useless determineDx
...
// but now without pizzasDiv in the for loop
...
// make updatePositions work with a requestAnimationFrame
var initiateScroll = 0;
function isScrolled() {
    var scrollValue = document.body.scrollTop / 1250;
    if (initiateScroll != scrollValue) {
        initiateScroll = scrollValue;
...
function updatePositions() {
  ...
  var phase = [];
  for (var i = 0; i < 5; i++) {
    phase.push(Math.sin(initiateScroll + (i % 5)));
  }
...
window.addEventListener('scroll', isScrolled, false);

// Generates the sliding pizzas when the page loads.
// Assuming a width max of 1980px and height max of 1280, 40 pizza is enough
